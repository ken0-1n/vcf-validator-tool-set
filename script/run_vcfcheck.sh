...
docker run --rm -it \
  ...
  -w /local \
  ${IMAGE_NAME} vcfcheck -f /local/reference/$REFERENCE_BASE /local/input/$INPUT_BASE \
  1> ${OUTPUT}/${INPUT_BASE}.vcfcheck.stdout.${DATE}.txt \
  2> ${OUTPUT}/${INPUT_BASE}.vcfcheck.stderr.${DATE}.txt
